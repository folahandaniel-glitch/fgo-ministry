...
- **Home, About, Wings, Children, Sermons, Bible School, Trivia, Forum,
  Events, Give, Testimonies, Register, Contact** — public pages
- **Bible School** — courses, enrollment, passcode- or payment-gated exams,
  DOCX question upload (mark the correct option with `*`), and downloadable
  Course Materials (docx, pptx, pdf) per course, also passcode/payment-gated
- **Children's Department** — sequential, illustrated Bible story lessons,
  completely free with no gating, optional attachments (docx, pptx, pdf,
  mp3, mp4) per lesson, a one-click Genesis story pack (Creation through
  Isaac), and the "Every Child, Every Potential, Every Future" banner image
- **Sermons** — text content plus an optional attached file (docx, pptx,
  pdf) or audio (mp3), playable inline when it's audio
- **Bible Trivia** — simple three-step flow (pick a topic → answer → see
  your score and the leaderboard), with a starter question pack and DOCX
  upload for admins
- **Forum** — topics and replies, open to everyone, with admin moderation
  (view and delete topics or individual replies)
- **Admin panel** (`admin.html`) — everything above, plus member/welfare
  list, event creation with ticketing, testimony moderation, giving
  overview, Paystack setup, About page content editor, and admin account
  management
...
**Not included yet, on purpose:** Pastor branch reporting (service reports,
per-branch pastor accounts). That's the next piece to build.
